...
export const makeReservation = async (req, res) => {
    ...
    try {
        ...
        const cita = await pool.query('SELECT * FROM medical_appointment WHERE appointment_id = ? AND appointment_status = true', [appointment_id]);
        if (cita.rows.length === 0) {
            return res.status(404).json({ error: 'La cita seleccionada ya no está disponible.' });
        }
        ...
        // Actualizar
        await pool.query('UPDATE medical_appointment SET appointment_status = false WHERE appointment_id = ?', [appointment_id]);
